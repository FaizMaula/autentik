      ...
      <div class="max-w-3xl mx-auto">
        ...
        <div class="glass-card-strong rounded-2xl p-6 md:p-8 animate-fade-in">
          <div class="text-center mb-6">
            <h1 class="text-2xl md:text-3xl font-bold text-[#222223] dark:text-[#FEFEFE] mb-2">{{ __('auth.registerTitle') }}</h1>
            <p class="text-gray-600 dark:text-gray-400 text-sm md:text-base">{{ __('auth.registerSubtitle') }}</p>
          </div>
        ...
        <form method="POST" action="{{ route('register') }}" novalidate>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <!-- Full Name Field (Left Top) -->
            <div>
              <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('auth.fullName') }}</label>
              <div class="relative">
                ...
                <input 
                  ...
                  name="name" 
                  value="{{ old('name') }}"
                  class="w-full pl-10 pr-4 py-3 glass-input rounded-lg focus:ring-2 focus:ring-[#B62A2D] focus:border-transparent transition-all duration-200 @error('name') border-red-500 @enderror" 
                  placeholder="{{ __('auth.fullNamePlaceholder') }}"
                  required 
                  autofocus
                />
              </div>
            ...
            <div>
              <label for="password_confirmation" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('auth.confirmPassword') }}</label>
              <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <svg class="w-5 h-5 fill-gray-700 dark:fill-gray-400" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zm-6 9c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2zM9 8V6c0-1.66 1.34-3 3-3s3 1.34 3 3v2H9z"/>
                ...
                <input 
                  ...
                  class="w-full pl-10 pr-12 py-3 glass-input rounded-lg focus:ring-2 focus:ring-[#B62A2D] focus:border-transparent transition-all duration-200" 
          ...
          <div class="flex flex-col items-center">
            <button 
              type="submit" 
              class="px-8 py-3 bg-[#222223] dark:bg-[#B62A2D] text-white rounded-lg font-semibold text-lg hover:bg-[#333334] dark:hover:bg-[#9a2426] transform hover:scale-[1.02] transition-all duration-300 shadow-lg hover:shadow-xl flex items-center justify-center gap-2 glow-red"
            >
              {{ __('auth.registerBtn') }}
            </button>
            ...
            <p class="mt-4 text-gray-600 dark:text-gray-400 text-center">
              {{ __('auth.hasAccount') }} 
              <a href="{{ route('login') }}" class="text-[#B62A2D] font-semibold hover:underline">{{ __('auth.loginLink') }}</a>
            </p>
          </div>
        </form>
      </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var fieldLabels = {
    'name': '{{ __("auth.fullName") }}',
    'email': '{{ __("auth.email") }}',
    'password': '{{ __("auth.password") }}',
    'password_confirmation': '{{ __("auth.confirmPassword") }}'
  };
  ...
  if (serverErrors) {
    var errors = JSON.parse(serverErrors.dataset.errors);
    if (errors.length > 0) {
      setTimeout(function() { showToast(errors); }, 100);
    }
  }
  ...
  form.addEventListener('submit', function(e) {
    ...
    if (password.value && password.value.length < 8) {
      errors.push('{{ __("auth.passwordTooShort") }}');
    ...
    if (password.value && confirmPassword.value && password.value !== confirmPassword.value) {
      errors.push('{{ __("auth.passwordMismatch") }}');
      addErrorState(confirmPassword);
